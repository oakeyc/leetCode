class Solution(object):
    def calculateMinimumHP(self, dungeon):
        ...
        self.mem = memioz
        helper = self.helper_min(dungeon, (0, 0), dungeon[0][0])
    ...
    def helper_min(self, d, position, curr_health, min_health):
        if (position == (len(d) - 1, len(d[0]) - 1)):
            return min(min_health, get(d, position)), get(d, position)
        ...
        curr_val = get(d, position)

        # where we are now
        curr_health += curr_val
       
        if len(d[right_pos[0]]) > right_pos[1]:  # do we want to go right

            if get(self.mem, right_pos) is None:  # have we done this before
                right_min, right_health = self.helper_min(d, right_pos, curr_health, min_health)
            else:
                right_min, right_health = get(self.mem, right_pos)

            right_health += curr_health
        
            right_min = min(right_min, min_health)

            move_right = min(right_health, right_min)
            right_tup = (right_min, right_health)

        if len(d) > down_pos[0]:
            if  get(self.mem, down_pos) is None:
                down_min, down_health = self.helper_min(d, down_pos, curr_health, min_health)
            else:
                down_min, down_health = get(self.mem, down_pos)

            down_health += curr_health
        
            down_min = min(right_min, min_health)

            move_down = min(down_min, down_health)
            down_tup = (down_min, down_health)

        if move_right is None:
            setmat(self.mem, position, down_tup)
            return down_tup
        if move_down is None:
            setmat(self.mem, position, right_tup)
            return right_tup
        elif move_right < move_down:  # negative
            setmat(self.mem, position, down_tup)
            return down_tup
        else:
            setmat(self.mem, position, right_tup)
