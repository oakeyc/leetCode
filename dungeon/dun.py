class Solution(object):
    ...
    def helper_min(self, position):
        ...
        if len(self.d[right_pos[0]]) > right_pos[1]:
            ...
            if get(self.mem, right_pos) is None:  # have we done this before
                right_min, right_health = self.helper_min(right_pos)

                setmat(self.mem, right_pos, (right_min, right_health))

            else:
                right_min, right_health = get(self.mem, right_pos)

            # to find out whether going right is a good choice, our acc health is our health + their health
            right_compare_health = right_health + curr_health

            right_compare_min = min(right_min + curr_health, curr_health)

            # whichever is worse is the one we need
            move_right = min(right_compare_health, right_compare_min)
            right_tup = (right_compare_min, right_compare_health)

        if len(self.d) > down_pos[0]:  # let's go down

            # yo same
            if  get(self.mem, down_pos) is None:
                down_min, down_health = self.helper_min(down_pos)
                setmat(self.mem, down_pos, (down_min, down_health))
            else:
                down_min, down_health = get(self.mem, down_pos)

            down_compare_health = down_health + curr_health
            down_compare_min = min(down_min + curr_health, curr_health)

            move_down = min(down_compare_health, down_compare_min)
            down_tup = (down_compare_min, down_compare_health)

        # this is the answer, the acc and health of this position
...
sol = Solution()
print("answer: ", sol.calculateMinimumHP([[1,-4,5,-99],
                                          [2,-2,-2,-1]]))
# printmat(sol.mem)
'''
[(-2, -2), (-4, -2), (-3, -1), (-100, -100)]
...
curhealth + right_health
min(cur_min, right_min + currhealth)
